...
*   **Manual Tag Management:** Users can add, edit, and categorize custom tags for individual images.
...
*   **Slideshow:** Automatic image cycling with adjustable delay.
*   **Model Management:** Toggle the tagging model's active state to manage memory usage.
*   **Clipboard Integration:** Support for pasting images directly into the preview area and copying images or tags to the system clipboard.
...
│   │   ├── manage_categories.py # Dialog for managing custom tag categories
│   │   ├── manage_directories.py # Dialog for managing directories and duplicates
│   │   ├── manage_tags.py  # Dialog for manual tag editing and assignment
...
*   **`gui/main_window.py`**: Defines the main `QMainWindow` for the application. It orchestrates the UI layout, connects signals and slots, manages the image gallery (loading, pagination, sorting), handles user interactions (clicks, search, slideshow), manages background workers, and coordinates features like model management and clipboard integration.
...
*   **`gui/dialogs/manage_categories.py`**: Defines the `ManageCategoriesDialog`, a dialog for creating, editing, and deleting custom tag categories.
...
*   **`gui/dialogs/manage_tags.py`**: Defines the `ManageTagsDialog`, allowing users to manually view, add, and modify tags and categories for a selected image.
...
*   **`gui/widgets/drag_drop_area.py`**: Implements a custom `QGraphicsView` widget used for the image preview panel. It handles drag-and-drop events (pasting, dragging out), manages image display (zooming, panning, navigation), and providing a context menu for actions like similarity search and slideshow control.
