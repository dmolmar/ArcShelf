...
class Database:
    ...
    def get_all_categories(self) -> List[str]:
        """Retrieves a list of all distinct tag categories currently in the database."""
        try:
            with self.lock:
                with sqlite3.connect(self.db_path) as conn:
                    cursor = conn.cursor()
                    cursor.execute("SELECT DISTINCT category FROM tags WHERE category IS NOT NULL AND category != ''")
                    return [row[0] for row in cursor.fetchall()]
        except sqlite3.Error as e:
            print(f"Database error retrieving categories: {e}")
            return []

    def get_all_tag_names(self) -> List[str]:
        """Retrieves a list of all distinct tag names currently in the database."""
        try:
            with self.lock:
                with sqlite3.connect(self.db_path) as conn:
                    cursor = conn.cursor()
            ...
            print(f"Database error retrieving tag names: {e}")
            return []

    def get_tags_with_manual_status(self, image_path: str) -> List[Tuple[str, str, int]]:
        """
        Retrieves tags for an image, including manual status.
        Returns: List of (name, category, is_manual) tuples.
        """
        image_id = self.get_image_id_from_path(image_path)
        if not image_id:
            return []
        ...
        try:
            ...
            print(f"Database error retrieving tags with status for {image_path}: {e}")
            return []